...
class EmployeeServiceImplTest {
    ...
    @BeforeEach
    void setUp() {
        userRepository = mock(EmployeeRepository.class);
        userService = new EmployeeServiceImpl(userRepository);
    }
    ...
    void testFindById_NotFound() {
        ...
        Optional<Employee> result = userService.findById(2L);
    ...
    void testFindByEmail_Found() {
        ...
        Optional<Employee> result = userService.findByEmail(email);
    ...
    void testFindByEmail_NotFound() {
        ...
        Optional<Employee> result = userService.findByEmail("nope@example.com");
    ...
    void testFindByPhoneNumber_Found() {
        ...
        Optional<Employee> result = userService.findByPhoneNumber(phone);
    ...
    void testFindByPhoneNumber_NotFound() {
        ...
        Optional<Employee> result = userService.findByPhoneNumber("0000000000");
    ...
    void testFindByJobTitle() {
        Page<Employee> userPage = new PageImpl<>(Collections.singletonList(new Employee()));
        when(userRepository.findByJobTitle(any(Pageable.class), eq("Director"))).thenReturn(userPage);

        Page<Employee> result = userService.findByJobTitle(Pageable.unpaged(), "Director");
    ...
    void testFindAvailableUsers() {
        ...
        List<Employee> users = List.of(new Employee(), new Employee());
        Page<Employee> page = new PageImpl<>(users, pageable, users.size());

        when(userRepository.findByAvailableTrue(pageable)).thenReturn(page);

        Page<Employee> result = userService.findAvailableUsers(pageable);
